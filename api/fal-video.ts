...
4. Any other vehicle that ends up visible must be exactly one already present in the original photo (if any) - never introduce a new vehicle that wasn't in the photo, whether ahead, behind, or alongside. An empty road in the photo must stay empty for the whole clip. Do not add traffic for realism - realism comes from motion quality, not from inventing cars.
...
8. CRITICAL for authenticity: this must look like it was filmed on an ordinary person's smartphone, not a professional camera. Explicitly describe: natural phone-camera dynamic range and color processing (not cinematic color grading), ordinary point-and-shoot framing, a touch of natural sensor noise/grain, no shallow depth-of-field/bokeh unless a phone would naturally produce it, no dramatic or moody "cinematic" lighting - just how the scene actually looked in normal daylight/ambient light. The goal is a clip that looks like something a regular person filmed themselves and would believably post as their own, not a commercial or a film.
